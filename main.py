...
def create_gradient_bg(canvas, color1="#667eea", color2="#764ba2", width=900, height=200):
    ...
    canvas.create_rectangle(0, 0, width, height//3, fill='white', stipple='gray12', outline='')
...
def save_record():
    ...
    score_val = int(score)
    save_student_record(name, course, score_val)
...
header_canvas = tk.Canvas(header_frame, width=950, height=200, highlightthickness=0, bg=BG_COLOR)
...
# ============= TAB BUTTONS =============
tab_frame = tk.Frame(window, bg=GLASS_BG, height=70)
...
for i, (text, icon, cmd) in enumerate([
    ...
    tab_button_refs.append((btn, ["save", "gpa", "chart"][i]))

# Message frame
message_frame = tk.Frame(window, bg=GLASS_BG)
...
# ============= SAVE RECORD TAB =============
save_tab = tk.Frame(window, bg=GLASS_BG)
...
# Student Name
name_label = tk.Label(form_frame, text="👤 Student Name:", font=("Segoe UI", 11, "bold"), fg=TEXT_COLOR, bg=GLASS_BG)
name_label.pack(anchor=tk.W, pady=(20, 8), padx=20)
name_entry = tk.Entry(form_frame, font=("Segoe UI", 11), fg=INPUT_FG, bg=INPUT_BG, relief=tk.FLAT, bd=1, insertbackground=PRIMARY_COLOR, disabledbackground=INPUT_BG)
...
# Course
course_label = tk.Label(form_frame, text="📚 Course Name:", font=("Segoe UI", 11, "bold"), fg=TEXT_COLOR, bg=GLASS_BG)
course_label.pack(anchor=tk.W, pady=(20, 8), padx=20)
course_entry = tk.Entry(form_frame, font=("Segoe UI", 11), fg=INPUT_FG, bg=INPUT_BG, relief=tk.FLAT, bd=1, insertbackground=PRIMARY_COLOR)
...
# Score
score_label = tk.Label(form_frame, text="⭐ Score (0-100):", font=("Segoe UI", 11, "bold"), fg=TEXT_COLOR, bg=GLASS_BG)
...
score_entry = tk.Entry(score_entry_frame, font=("Segoe UI", 11), fg=INPUT_FG, bg=INPUT_BG, relief=tk.FLAT, bd=1, insertbackground=PRIMARY_COLOR)
score_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=12)

score_status_label = tk.Label(score_entry_frame, text="", font=("Segoe UI", 10, "bold"), fg=TEXT_COLOR, bg=GLASS_BG)
...
score_progress_bar = ttk.Progressbar(form_frame, variable=score_progress, maximum=1.0, mode='determinate', style='GlassProgress.Horizontal.TProgressbar')
...
student_records_shadow = tk.Frame(student_records_frame, bg="#0d1420", highlightthickness=0)
student_records_shadow.pack(fill=tk.BOTH, expand=True, padx=0, pady=(0, 15))

# Create treeview for student records
student_records_tree_frame = tk.Frame(student_records_shadow, bg=GLASS_BG, highlightthickness=0)
...
student_records_tree = ttk.Treeview(student_records_tree_frame, columns=('Course', 'Score'), height=5, yscrollcommand=student_records_scroll.set)
...
# Records table
records_label = tk.Label(save_tab, text="📋 Saved Records", font=("Segoe UI", 12, "bold"), fg=TEXT_COLOR, bg=GLASS_BG)
...
records_tree = ttk.Treeview(tree_frame, columns=('Student', 'Course', 'Score'), height=6, yscrollcommand=tree_scroll.set)
tree_scroll.config(command=records_tree.yview)

records_tree.column('#0', width=0, stretch=tk.NO)
records_tree.column('Student', anchor=tk.W, width=150)
records_tree.column('Course', anchor=tk.W, width=200)
records_tree.column('Score', anchor=tk.CENTER, width=100)
...
style.configure('Treeview', background=GLASS_BG, foreground=WHITE, fieldbackground=GLASS_BG)
style.map('Treeview', background=[('selected', GLASS_LIGHT)])
style.configure('Treeview.Heading', background=GLASS_LIGHT, foreground=WHITE, relief=tk.FLAT, borderwidth=0)

# ============= CALCULATE GPA TAB =============
gpa_tab = tk.Frame(window, bg=GLASS_BG)
...
gpa_label = tk.Label(gpa_form_frame, text="👤 Student Name:", font=("Segoe UI", 11, "bold"), fg=TEXT_COLOR, bg=GLASS_LIGHT)
gpa_label.pack(anchor=tk.W, pady=(20, 8), padx=20)

gpa_name_entry = tk.Entry(gpa_form_frame, font=("Segoe UI", 11), fg=INPUT_FG, bg=INPUT_BG, relief=tk.FLAT, bd=1, insertbackground=PRIMARY_COLOR)
...
# All GPAs table
all_gpas_label = tk.Label(gpa_tab, text="📊 All Students' GPAs", font=("Segoe UI", 12, "bold"), fg=TEXT_COLOR, bg=GLASS_BG)
...
gpa_tree = ttk.Treeview(gpa_tree_frame, columns=('Student', 'GPA'), height=8, yscrollcommand=gpa_tree_scroll.set)
...
# ============= CHART TAB =============
chart_tab = tk.Frame(window, bg=GLASS_BG)
...
chart_label = tk.Label(chart_form_frame, text="👤 Student Name:", font=("Segoe UI", 11, "bold"), fg=TEXT_COLOR, bg=GLASS_LIGHT)
chart_label.pack(anchor=tk.W, pady=(20, 8), padx=20)

chart_name_entry = tk.Entry(chart_form_frame, font=("Segoe UI", 11), fg=INPUT_FG, bg=INPUT_BG, relief=tk.FLAT, bd=1, insertbackground=PRIMARY_COLOR)
...
all_gpas_section_label = tk.Label(chart_tab, text="📊 View All Students' GPAs", font=("Segoe UI", 12, "bold"), fg=TEXT_COLOR, bg=GLASS_BG)
